...
#+BEGIN_SRC latex tangle: st-loesung-08.tex
\subsubsection{a)}
Die Wahrscheinlichkeit für die unterschiedlichen Augenzahlen im 2. Wurf ändern sich nicht durch den ersten, 
und liegt daher bei $\frac{1}{6}$ für jede mögliche Augenzahl. Die 6 möglichen Ergebnisse mit P(Y=y)>0 nach dem 2 Wurf
sind daher lediglich um k verschoben.\\
$\Rightarrow P(Y=y) = \begin{cases} \frac{1}{6} &\mbox{falls } k < y \leq 6+k \\ 0 &\mbox{sonst} \end{cases}$ \\
#+END_SRC
**** b)
...
#+BEGIN_SRC latex tangle: st-loesung-08.tex
\subsubsection{b)}
Sei Z die Augenzahl des 2. Wurfes, so gilt $P[Z] = \frac{1}{6}$ und $E[Z] = \frac{1}{6}(1+2+3+4+5+6) = 3,5$
wobei ein bekanntes X keinerlei Einfluss haben kann.
Es gilt daher: $g(k) := E[Y|X=k] = E[Z] + k = 3,5 + k$
#+END_SRC
...
#+BEGIN_SRC latex tangle: st-loesung-08.tex
\subsubsection{c)}
Sei Z wieder die Augenzahl des zweiten wurfes. Da X für E[Y] unbekannt ist, gilt E[X] = E[Z] = 3,5. Da sich beide Werte nicht
beeinflussen, hat Y also die Summe der beiden Augenzahlen den Erwartungswert E[Y] = E[X] + E[Z] = 3,5 + 3,5 = 7 \\
