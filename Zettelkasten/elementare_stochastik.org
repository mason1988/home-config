...
*** TODO eine der alternativen muss rausgestrichen werden
*** Lösung
#+BEGIN_SRC latex :tangle st-loesung-11.tex
\subsection{Aufgabe 1}
Statistisches Modell: \\
Das statistische Modell sei $(\mc{X}, \mc{F}, P_{\theta} : \theta \in \Theta)$ mit: \\ 
$\mc{X} = \{0,1\}^{10}$ \\ %alternativ: Sei Y der Ausgang des Experiments(=001101...), \mc{X} = [0,10] \subset \N = \sum_{y\in Y}y
...
Likelihood Funktion: \\
$L(\theta | 0,0, 1,1, 0,1, 0,0, 0,1) = P_{\theta}( 0,0, 1,1, 0,1, 0,0, 0,1)$ \\ 
$= \Pi_{x \in \mc{X}}P_{\theta}(x)$ \\
alternativ, falls x = Anzahl: $= \binom{10}{x} \theta^{x} * (1-\theta)^{10-x}$ %
Maximum likelihood Schätzer: \\
Da die logarithmus Funktion von $L(\theta)$ an der selben Stelle ihr maximum erreicht, reicht es, diese zu maximieren. \\
$\Ri log(L(\theta)) = \sum_{x \in \mc{X}}log(P_{\theta}(x))$ \\ % <= keine Ahnung, der Logarithmus ist nur geraten
%ab hier alles alternative
alternativ, falls x = Anzahl: $= log(L(\theta))  = log(\binom{10}{x}) + x*log(\theta) + (10-x)*log(1 - \theta)$ \\
$\Ri \fr{d}{d\theta}log(L(\theta)) = \fr{x}{\theta} - \fr{10-x}{1 - \theta}$  %immernoch alternative
...
*** Referenzen
** footer
#+begin_src latex :tangle st-zettel-11.tex
\end{document}
#+end_src
