...
Beweis:
$A \tilde B \Lri \fr{A}{B} \m{abl}, \fr{B}{A} \m{abl} \Ri A = (PT)^{J \cup Q} \bs \tilde$ Boole'sche Algebra, $e = \bar{(PT)^{J\cup Q}_0}$ = ableitbare Formeln
...
Sei $rgA = n>0 \Ri_{Peano} A = \begin{cases} \bar{B} &\m{Negation} rgA = rgB+1 \Ri rg B < n \\ B \sharp C &\m{Junktion} rgA = rgB+rgC+1 \Ri rgB < n, rg C < n \\ \sharp_xB &\m{Quantion} rgA = rgB+1 \Ri rgB < n \end{cases}$

$A = \bar{B} \Ri \iota^{\w}A = \iota^{\w}\bar{B} = 1 - \iota^{\w}B =_{ind} 1 - \chi[B] = \chi(e-[B]) = \chi \bar{[B]} = \chi [\bar{B}] = \chi[A]$
