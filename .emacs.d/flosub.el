...
(defun flosub-prev-readall() (interactive)
      (setq flosub-exp-end (point))
      (search-backward-regexp "[\n ]")
      (setq flosub-exp-st (+ 1 (point)))
      (setq end (buffer-substring-no-properties flosub-exp-st flosub-exp-end))

      (setq flosub-exp-end-tmp (- (point) 1))  ;vlt.  auch nicht
      (search-backward-regexp "[\n ]")
      (setq flosub-exp-st (+ 1 (point)))
      (setq start (buffer-substring-no-properties flosub-exp-st flosub-exp-end-tmp))

      (forward-char) ;da wir nicht das leerzeichen suchen
