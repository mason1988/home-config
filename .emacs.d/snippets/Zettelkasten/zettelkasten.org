...
* Zettelverwaltung
** Zettel6 *Elementare Stochastik*
** Zettel9 *Logik*
** Zettel9 *Softwaretechnik*
** Zettel6 *Theoretische Informatik*
* Elementare Stochastik						     :ATTACH:
  :PROPERTIES:
...
** Zettel-06							     :ATTACH:
   ...
   :ID:       58dff94f-e04a-49a8-80bc-49bc8037c046
   :END:
*** Dateien
...
**** Bedingter Erwartungswert				       :st:knowledge:
A,B Ereignisse; Y Zufallvariable
$Dropbox/Zettelkasten/P(A|B) = \frac{P(A \cap B)}{P(B)}$ Ws A wenn B bekannt
...
** Zettel-08
[[~/Dropbox/st/st-zettel-08/st-zettel-08.pdf][st-zettel-08.pdf]]
[[~/Dropbox/st/st-zettel-08/st-loesung-08.tex][st-loesung-08.tex]]
[[~/Dropbox/st/st-zettel-08/st-loesung-08.pdf][st-loesung-08.pdf]]
** Zettel-09
...
*** Shortcuts                                                     :shortcuts:
   ...
   C-k / C-j nächster / letzter Header auf selber Stufe
** Vim
...
*** kleinkram
c löscht geht aber direkt auf insert
...
** yas
- new snippet, kram reinpasten/ ${1} ... für var, reload, insert buffer
*** Beispiel Snippet
# -*- mode: snippet -*-
# name: leftrightarrow
# key: Lra
...
* Todo List
** DONE java einrichten, sdk downloaden und mal ein projekt öffnen
** DONE emacs datei reparieren
** DONE Org Mode Steuerung sammeln / optimieren
   - z.B. das hin und her "senden" von zeug
   - das navigieren ist immernoch ätzend

     1. vermehrt vim marks benutzen

     2. Die Suche Für Konkrete Punkte Benutzen
** DONE git in emacs auch für andere Sachen benutzen
** DONE schnelleres shortcut für rodentbane
** DONE shortcuts automatisch einbinden
** DONE Berechtigungen ändern
** DONE die Shortcut Präfixe ändern
   oder auch nicht, müsste nochmal gebrainstormed werden
   so ohne weiteres auch gar nicht möglich, statt dessen können wir
   aber manuell für die wichtigsten Funktionen neue Shortcuts einsetzen
   ok, bla... funktioniert doch. C-x ist erstmal auf C-ü geändert
  ansonsten wäre die Hauptsache auch erstmal, die 2. steuertaste überhaupt
   auch mal zu benutzen >.>
** TODO Grep für Pdf's?
** DONE autosave für Emacs verbessern (was spuckt google da für autosave aus?)
     - Von interesse: Speicherrythmus, Speicherordner, konkrete Wiederherstellung u. Warnung
** DONE Session Management bei Emacs
     - neuen Unterpunkt anlegen und klare Entscheidungen und Kriterien treffen
       und die Shortcuts notieren
     
     - 
** DONE Jump Befehle Emacs
     - gesucht sind Sachen wie das Springen zur nächsten leeren Zeile,
       oder z.B. nächstes Zeichen in aktueller Spalte
       So ganz abgefahrene Sachen hat Emacs dann auch nicht (im standard)
       aber in elisp ist das sehr schnell zu implementieren. Einfach mal beobachten
       was sich in der Praxis anbietet
** DONE Zeug automatisieren mit Emacs ( irgend sowas mal googlen )
** DONE Entscheiden ob weiter Git oder SVN
     - Entscheidung dann umsetzen bzw. Befehle und Workflow fixieren
** TODO in orgmode gleichzeitig datei erstellen und link setzen
** DONE Vim oder Emacs: einfach nur sprung zu nächsten vorkommen (nicht nur zeile, ohne komplette suche)
   für einzelnes zeichen wird das wohl keiner machen, liesse sich zwar schnell in elisp implementieren,
   mit increment-search aber der zusätzliche tastendruck auf enter machts dann aucht nicht aus.
   Vielleicht das starten der suche im evil mode auf ein einzelnes zeichen legen?
** TODO Brainstorming Browser-Session-Management über Org? (Hauptsächlich sammeln von ReadItLater
** DONE Abkürzungen für die unterschiedlen PasteFunktionen in Emacs finden
  normaler Paste (Vim) reicht da aus.
** TODO Bookmarks um stellen zu markieren *an* die kopiert wird, bzw. *von* wo automatisch kopiert wird
** DONE weitere Hilfsbefehle zur Liste hinzufügen
** TODO Sessions durchgehen
** DONE auf dem netbook die shortcuts etc. einrichten
   ich denke das darf mit dem git kram als gelöst
   betrachtet werden
** TODO translator zu kupfer/synapse
** TODO kupfer die einzelnen Terminals finden lassen -> Cmus z.B.
** DONE Standardsuhcnmaschine in Firefox ändern
** DONE einfügen aus Cache in Emacs
   die bookmarks können sowas auf jeden fall
** TODO shortcuts für konkrete Fenster
** DONE Key Script korrigieren
   - gibt wohl nach wie vor Probleme mit den Shells
     (nach nem su läufts)
** DONE bookmarks im Orgmode? (Ohne verschiebung der Zeilen)
** TODO ysnippet erweiterungen sichten und kurze zusammenfassung der Benutzung schreiben
** DONE Scrollen in der Konsole mit den üblichen Shortcuts
   mit shift und bildlauf geht schon vieles. Wenn der Rest über die eshell
	standardnamen stehen ja schon, verschiebung will nur noch nicht
   läuft, würde es reichen, die näher zu klären.
   - jetzt funktionieren die normalen tasten
   - da der evil-modus läuft, reicht ein [esc] um frei zu navigieren und
     dann sogar in den Visual Mode zu wechseln
* Sessions
http://wiki.ubuntuusers.de/Fenstermanager
http://wiki.ubuntuusers.de/GNOME_Fenstermanager_auswechseln
** Desktop Enviroments
http://wiki.ubuntuusers.de/Desktophttp://wiki.ubuntuusers.de/Desktop
http://wiki.ubuntuusers.de/Eigene_Desktopumgebung
** Theorie
https://wiki.archlinux.org/index.php/Window_Manager
http://en.wikipedia.org/wiki/Dynamic_window_manager
...
** org-mode
siehe hierzu auch [[http://doc.norang.ca/org-mode.html][Your Life in Plaintext]]
Bisher steht eine wirkliche Einführung in Knowledge/Task management
mit dem OrgMode noch aus
